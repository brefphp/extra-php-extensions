...
define build_docker_image
	docker buildx build -t bref/$(notdir ${1})-php-${2} \
...
test: docker-images
	...
	for dir in layers/${layer}; do \
		for php_version in $(call resolve_php_versions,$${dir}); do \
			...
			echo "###"; \
			docker buildx build --build-arg PHP_VERSION=$${php_version} --build-arg TARGET_IMAGE=$$(basename $${dir})-php-$${php_version} -t bref/test-$$(basename $${dir})-$${php_version} $(CACHE_FLAGS) --load tests ; \
			docker run --entrypoint= --rm -v $$(pwd)/$${dir}:/var/task bref/test-$$(basename $${dir})-$${php_version} /opt/bin/php /var/task/test.php ; \
			if docker run --entrypoint= --rm -v $$(pwd)/$${dir}:/var/task bref/test-$$(basename $${dir})-$${php_version} /opt/bin/php -v 2>&1 >/dev/null | grep -q 'Unable\|Warning'; then exit 1; fi ; \
			echo ""; \
			echo " - Test passed"; \
			echo ""; \
		done \
