...
if (!extension_loaded('pdo_sqlsrv')) {
    echo test . phpsprintf('FAIL: PDO sqlsrv extension not loaded') . PHP_EOL;
    exit(1);
}
...
if (!sqlsrv_connect('localhost', ['LoginTimeout' => 1])) {
    foreach (sqlsrv_errors() as $error) {
        if ($error['SQLSTATE'] == '01000') {
            echo test . phpsprintf('FAIL: sqlsrv extension library not loaded. %s', $error['message']) . PHP_EOL;
            exit(1);
        }
    }
}
